...
combined1 <- plot_grid(state_filled, state_filled2, labels = c("Frequency of UFO citings per state", "Proportion of UFO citings per state by population"), align = "hv", nrow = 2, hjust = -.05)

#Adding a caption
ggdraw(add_sub(combined1, "Data from National UFO Reporting Center and US Census Bureau", x = .5, y = 1, hjust = .3, vjust = 0.5, fontfamily = "",
...
freq_y <- c(45.6, 44, 45.5, 44.8, 45.2)
freq_x <- c(-122.1, -122.7, -123.3, -122.1, -122.2)
...
or_plot1 <- plot_title_freq + 
  geom_shadowtext(data = freq_top5, aes(label = names, x = x, y = y), inherit.aes = FALSE, size = 2, bg.colour='violetred4') + geom_point(aes(x = long, y = lat), data = cities, inherit.aes = FALSE) + 
    geom_text_repel(aes(x = long, y = lat, label = city), data = cities, inherit.aes = FALSE)
```
...
or_plot2 <- plot_title  + 
  geom_shadowtext(data = prop_top5, aes(label = names, x = x, y = y), inherit.aes = FALSE, size = 2, bg.colour='violetred4')
...
#combining plots 1A and 1B into one plot side by side
combined2 <- plot_grid(or_plot1, or_plot2, labels = c("", ""), align = "hv", nrow = 2, hjust = -.05)
